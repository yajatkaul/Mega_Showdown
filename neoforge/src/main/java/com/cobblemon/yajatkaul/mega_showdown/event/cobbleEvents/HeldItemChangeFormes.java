...
public class HeldItemChangeFormes {
    ...
    public static void therianEvent(HeldItemEvent.Post event){
        ...
        if(event.getReceived().is(FormeChangeItems.REVEAL_GLASS)){
            new StringSpeciesFeature("mirror_forme","therian").apply(pokemon);
            EventUtils.playEvolveAnimation(pokemon.getEntity());
        } else if (!event.getReceived().is(FormeChangeItems.REVEAL_GLASS) && event.getReturned().is(FormeChangeItems.REVEAL_GLASS)) {
