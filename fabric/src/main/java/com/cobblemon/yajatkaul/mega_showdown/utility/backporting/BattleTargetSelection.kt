...
class BattleTargetSelection(
    ...
    val backButton = BattleBackButton(x + 9F, MinecraftClient.getInstance().window.scaledHeight - 22F)
    ...
    val baseTiles = targets.mapIndexed { index, target ->
        ...
        if (verticalAligned) {
            x =
                this.x + (MinecraftClient.getInstance().window.scaledWidth / 2) + if (isAlly) -(TARGET_WIDTH + MOVE_HORIZONTAL_SPACING) else MOVE_HORIZONTAL_SPACING
            y =
            ...
            x =
                this.x + MinecraftClient.getInstance().window.scaledWidth / 2 - (TARGET_WIDTH + MOVE_HORIZONTAL_SPACING) * teamSize / 2 + MOVE_HORIZONTAL_SPACING + fieldPos * TARGET_WIDTH
    ...
    override fun renderWidget(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        blitk(
            ...
            x = x,
            y = y,
            width = MinecraftClient.getInstance().window.scaledWidth,
        ...
        drawScaledText(
            context = context,
            text = text,
            x = (MinecraftClient.getInstance().window.scaledWidth - width) / 2,
